...
singularity exec -B ${ROOTDIR}:${ROOTDIR} ${IMAGES}/bowtie2machalen.img bowtie2 -x ${REFGENOME}/Human_GRCh38/Human_GRCh38 -U example_trimmed.fq -p 4 --no-unal -S ${BAMDATA}/output_input.sam
...
singularity exec -B ${ROOTDIR}:${ROOTDIR} ${IMAGES}/deepTools.simg /tool_deps/_conda/pkgs/samtools-1.4.1-0/bin/samtools sort unique_input.bam -o sorted_unique_example.bam

singularity exec -B ${ROOTDIR}:${ROOTDIR} ${IMAGES}/deepTools.simg /tool_deps/_conda/pkgs/samtools-1.4.1-0/bin/samtools index sorted_unique_example.bam
